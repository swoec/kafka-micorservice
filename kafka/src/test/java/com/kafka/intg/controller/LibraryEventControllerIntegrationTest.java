...
public class LibraryEventControllerIntegrationTest {
	...
	void putLibraryEvent() {
		//given 
		Book book = Book.builder().bookId(123).bookName("kafka").bookAuthor("Wang").build();
		LibraryEvent libraryEvent = LibraryEvent.builder().libraryEventId(234).book(book).build();
		...
		ConsumerRecord<Integer, String> customerRecord = KafkaTestUtils.getSingleRecord(consumer, "library-events");
		String expected = "{\"libraryEventId\":234,\"libraryEventType\":\"UPDATE\",\"book\":{\"bookId\":123,\"bookName\":\"kafka\",\"bookAuthor\":\"Wang\"}}";
		String value = customerRecord.value();
		assertEquals(expected, value);
	}
